...
```{r}
ggplot(dat_tokenized_unnested %>% group_by(sentiment, sentiment_num) %>% count(score_sentiment)) +
  ggridges::geom_ridgeline(aes(score_sentiment, y = factor(sentiment_num), height = n,
                               scale = n,
...
ggplot(dat_tokenized_unnested) +
  geom_density(aes(score_sentiment)) +
  # geom_point(aes(sentiment_num, y = score_sentiment))
  facet_wrap(~ sentiment)
```
