...
```{r}
12345 %>% get_classification_batch()
```
...
And then plot the word's sentiment as scored on the `AFINN` scale.
...
We might be interested in phrases that follow specific words like "use" or "don't use". We might also want to pull out a category or categories if they exist in the phrase. We can make something reusable like:
...
By default we won't append "the"
