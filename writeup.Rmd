...
MonkeyLearn is a fantastic tool for performing sentiment analyses tailored to a particular use case. In this post I'll give an example of how you might use the `monkeylearn` R package to conduct a seamless sentiment analysis of consumer product reviews. I'll be examining reviews of the popular colaboration tool [Slack](https://slack.com/) on the product review site [Capterra](https://www.capterra.com/).
...
In this case, we'll want each review's overall rating, all of its sub-ratings, as well as the text, or content, of the review. What I'm calling sub-ratings are the reviews' ratings of certain aspects of the service like Customer Support, Ease of Use, and Value for Money. Like the overall rating, users have the option to give these aspects a rating from 1 to 5 out of 5.
...
However, through a bit of investigation of the "Show more reviews" button in the Chrome inspector, I saw that inside the link's `a` tag is a [`data-url`](https://en.wikipedia.org/wiki/Data_URI_scheme) parameter set to `"/gdm_reviews?page=1&product_id=135003"`. 
...
Now all we need to do is read them in and stick them together into a dataframe.
...
We'll also create a unique identifier for each review, by first combining `review_num` and `page_num`. (We could have hashed these using e.g. the `digest` package but stringing them together into `doc_identifier` gives us more understandable information at a glance about the review.)
...
Then we can assign each piece of content a unique ID, starting with 1 for our oldest review.
...
ggplot(sentiment_breakdown) +
  ...
  geom_text(aes(sentiment, sentiment_prop + 0.03, label = round(sentiment_prop*100, digits = 2)),
            fontface = "italic", size = 3) +
  scale_y_continuous(labels = scales::percent) +
  ggtitle("Sentiment Breakdown, Overall") +
  labs(x = "Sentiment", y = "Proportion", fill = "Sentiment") +
  theme_bw()
```
...
There is very little difference in overall ratings of the product. (It's important to remember that there is a one:many relationship between ratings and opinion units here; each review gets a at most single rating, but reviews are later parcelled into multiple opinions.)
...
(Don't ask me why yummy is more positive than yucky is negative `r emo::ji("laughing")`.) Anyway, let's join this on our dataset by "word".
...
If we're interested in doing an analysis of words that belong to opinion units that were tagged with certain categories or sentiments, we're going to need back our measure of each of those, which were labeled at the opinion unit level. Luckily we can unnest our `dat_tokens_unnested_tfidf` dataframe in place of doing any joining.

```{r}
dat_tokens_unnested_unnested <- 
  dat_tokens_unnested %>% 
  unnest()
```
...
```{r}
lm(data = dat_tokens_unnested_unnested %>% drop_na(sentiment_num, score_sentiment), 
...
And then plot the word's sentiment as scored on the `AFINN` scale. The dashed horizontal line represents the mean sentiment score for words in our dataset.
...
ggplot(dat_tokens_unnested_first_letter) +
  ...
  geom_hline(data = dat_tokens_unnested_first_letter, yintercept = mean(dat_tokens_unnested_first_letter$score_sentiment, na.rm = TRUE), linetype = "dashed") +
...
We might be interested in phrases that follow specific words like "use" or "don't use". We might also want to pull out a category or categories if they exist in the phrase. 

We'll make a regex for all of the categories MonkeyLearn has identified:
...
search_for <- function(df = dat_clean, col = content, word = "love", append_the = FALSE) {
  ...
  out <- 
    ...
    mutate(
      ...
      phrase_categories = str_extract_all(phrase, category_reg) %>% replace_y() %>% str_c(collapse = ", ")
    ) 
...
A question we might be interested in is: which words are most distinctive to opinion units tagged with each category?
...
Is the desktop app or mobile app mentioned more in P-Q-R compaints?
...
Here we've built a relatively straightfoward pipeline for an analysis of web data. We grab and clean our raw data, feed it to MonkeyLearn for extraction and classification, and then analyze the results. MonkeyLearn allows us to abstract out the machine learning and plug into a simple and reliable API. 
